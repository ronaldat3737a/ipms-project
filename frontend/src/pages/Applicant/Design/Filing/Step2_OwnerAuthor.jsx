...
const Step2_OwnerAuthor = ({ isRevision = false }) => {
  ...
  const addAuthor = () => {
    // Sử dụng Optional Chaining để bảo vệ mảng authors
    const currentAuthors = formData?.authors || [];
    const newAuthor = { id: Date.now(), fullName: "", nationality: "Việt Nam", idNumber: "" };
    updateFormData({ authors: [...currentAuthors, newAuthor] });
  };

  const removeAuthor = (id) => {
    const newAuthors = formData?.authors?.filter(a => a.id !== id) || [];
  ...
  // ===========================================================================
  // 2. KHỐI NỘI DUNG FORM (FormFields) - GIỮ NGUYÊN 100% GIAO DIỆN CŨ CỦA BẠN
  // ===========================================================================
  const FormFields = (
    <div className={`space-y-12 ${isRevision ? "" : "max-w-4xl mx-auto"}`}>
      {/* Chỉ hiện tiêu đề khi nộp đơn mới */}
      {!isRevision && <h1 className="text-3xl font-bold">2. Chủ đơn & Tác giả</h1>}

      {/* PHẦN 1: THÔNG TIN CHỦ ĐƠN */}
      <section className="p-8 rounded-2xl border border-gray-200 space-y-6 relative text-left">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <InputGroup label="Điện thoại" name="ownerPhone" value={formData?.ownerPhone || ""} required onChange={(e) => updateFormData({ownerPhone: e.target.value})} />
            <InputGroup label="Email" name="ownerEmail" value={formData?.ownerEmail || ""} required onChange={(e) => updateFormData({ownerEmail: e.target.value})} />
          </div>
      ...
      <section className="p-8 rounded-2xl border border-gray-200 space-y-6 text-left">
        <div className="flex justify-between items-center mb-2">
            ...
            <div key={author.id} className="flex items-end gap-3 group">
              <div className="flex-grow grid grid-cols-3 gap-3 bg-gray-50/50 p-4 rounded-xl border border-dashed border-gray-200">
                ...
                <button 
                  ...
                  className="p-2 mb-2 text-gray-300 hover:text-red-500 transition-colors"
  ...
  // ===========================================================================
  // 3. LOGIC HIỂN THỊ (RENDER) LINH HOẠT
  // ===========================================================================
  
  // Nếu là chế độ Sửa đơn (Revision): Chỉ trả về FormFields, không kèm Header/Sidebar
  if (isRevision) {
    return (
      <div className="bg-white p-0 animate-in fade-in duration-500">
        <div className="mb-6 border-b pb-4 text-left">
          <h2 className="text-xl font-bold text-slate-800">2. Chủ đơn & Tác giả</h2>
        </div>
        {FormFields}
      </div>
    );
  ...
  // Chế độ nộp đơn mới: Trả về đầy đủ giao diện gốc của bạn
  return (
