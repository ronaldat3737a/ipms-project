...
const Step6_Submission = () => {
   ...
   */
  /**
   * Logic nộp hồ sơ chính thức - Đã sửa để gửi File thực tế
   */
  const handleSubmit = async () => {
    ...
    try {
      // 1. Tạo đối tượng FormData để đóng gói cả Chữ và File
      const data = new FormData();

      // 2. Đưa dữ liệu chữ (JSON) vào ngăn "patentData"
      data.append("patentData", JSON.stringify(formData));

      // 3. Đưa các file vật lý từ Step 3 vào ngăn "files"
      formData.attachments.forEach((item) => {
        if (item.file) {
          // 'file' là đối tượng File gốc chúng ta đã lưu ở Step 3
          data.append("files", item.file);
        }
      });

      // 4. Gửi yêu cầu POST tới API
      const response = await fetch("http://localhost:8080/api/patents/submit", {
        method: "POST",
        // LƯU Ý: Tuyệt đối KHÔNG set Header Content-Type.
        // Trình duyệt sẽ tự động thêm multipart/form-data khi thấy body là FormData.
        body: data,
      });
      ...
      if (response.ok) {
        ...
        // Xóa sạch dữ liệu để tránh nộp đơn mới bị dính đơn cũ
        clearFormData();

        // Điều hướng đến trang thành công và truyền mã số đơn appNo
        navigate("/applicant/patent/success", {
          state: { appNo: result.appNo || "Đang xử lý" },
        });
      } else {
        const errorData = await response.text();
        alert(`Nộp đơn thất bại: ${errorData || "Lỗi máy chủ"}.`);
      }
    } catch (error) {
      console.error("Lỗi kết nối API:", error);
      alert("Không thể kết nối tới Backend. Hãy kiểm tra server Spring Boot!");
    } finally {
          ...
          <div className="w-full max-w-2xl">
            ...
            <div className="border-2 border-black rounded-[32px] p-10 space-y-8 bg-white shadow-sm">
              ...
              <p className="text-sm text-center text-gray-600 leading-relaxed px-4">
                Vui lòng kiểm tra kỹ các thông tin đã khai báo. Sau khi nhấn
                "Nộp đơn", bạn sẽ không thể chỉnh sửa hồ sơ cho đến khi có thông
                báo từ Cục Sở hữu trí tuệ.
              </p>
                ...
                <label className="flex items-start gap-4 cursor-pointer group">
                  ...
                  <div className="space-y-4">
                    ...
                    <ul className="space-y-3 list-disc list-outside ml-4 text-xs font-medium text-gray-600 leading-relaxed">
                      <li>
                        Mọi thông tin trong đơn và các tài liệu đính kèm là hoàn
                        toàn trung thực và chính xác.
                      </li>
                      <li>
                        Tôi là người có quyền nộp đơn hoặc được ủy quyền hợp
                        pháp để thực hiện thủ tục này.
                      </li>
                      <li>
                        Tôi hoàn toàn chịu trách nhiệm trước pháp luật về tính
                        xác thực của hồ sơ đã nộp.
                      </li>
                    </ul>
                  </div>
                </label>
