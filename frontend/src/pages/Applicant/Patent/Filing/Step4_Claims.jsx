...
const Step4_Claims = () => {
  ...
  // Thêm điểm mới
  const addClaim = () => {
    const newId = formData.claims.length > 0 ? Math.max(...formData.claims.map(c => c.id)) + 1 : 1;
    const newClaim = { 
      id: newId, 
      type: "Độc lập", 
      content: "", 
    ...
    updateFormData({ claims: [...formData.claims, newClaim] });
  };

  // Xóa điểm
  const removeClaim = (id) => {
    updateFormData({ claims: formData.claims.filter(c => c.id !== id) });
  };

  // Cập nhật nội dung điểm
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col font-sans text-gray-800">
      ...
      <div className="flex flex-grow overflow-hidden">
        ...
        {/* Nội dung chính Step 4 */}
        <main className="flex-grow p-12 overflow-y-auto bg-white">
          <div className="max-w-4xl mx-auto space-y-8">
            <h1 className="text-3xl font-bold italic">4. Yêu cầu bảo hộ</h1>
            ...
            <div className="space-y-6">
              {formData.claims.map((claim, index) => (
                <div key={claim.id} className={`p-8 border rounded-2xl relative transition ${claim.status === "Cần chỉnh sửa" ? "border-red-100 bg-red-50/5" : "border-gray-100 shadow-sm"}`}>
                    ...
                    <div className="flex-grow space-y-6">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative w-48">
                          ...
                          >
                            <option>Độc lập</option>
                            <option>Phụ thuộc</option>
                          </select>
                      ...
                      {/* Thông báo lỗi & Tham chiếu nếu là điểm phụ thuộc */}
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="relative">
                              ...
                              >
                                ...
                                {formData.claims.filter(c => c.id !== claim.id).map(c => (
                                  <option key={c.id} value={c.id}>Điểm {formData.claims.findIndex(f => f.id === c.id) + 1}: {c.content.substring(0, 50)}...</option>
                                ))}
                              </select>
            ...
            <p className="text-xs text-gray-400 font-medium">
              Tổng số điểm: <span className="text-gray-800 font-bold">{formData.claims.length}</span>. (Phí sẽ được tính dựa trên số lượng điểm này tại Bước 5.)
            </p>
            ...
            <div className="flex justify-end gap-4 pt-10">
              ...
              <button onClick={() => navigate("/applicant/patent/step5")} className="flex items-center gap-2 px-8 py-3 bg-blue-500 text-white rounded-xl font-bold hover:bg-blue-600 transition shadow-md">
