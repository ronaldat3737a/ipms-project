...
const Step1_GeneralInfo = ({ isRevision = false }) => {
  ...
  // ===========================================================================
  // KHỐI NỘI DUNG FORM (Giữ nguyên 100% logic và giao diện cũ của bạn)
  // ===========================================================================
  const FormContent = (
    <div className={`${isRevision ? "" : "max-w-4xl"}`}>
      {/* Chỉ hiện tiêu đề lớn khi nộp đơn mới */}
      {!isRevision && <h1 className="text-3xl font-bold mb-10">1. Thông tin chung</h1>}
      ...
      <div className="space-y-10">
        ...
        <section className="space-y-3 text-left">
          <h3 className="text-sm font-bold text-gray-700 uppercase tracking-wider">
            Tiêu đề sáng chế/Giải pháp hữu ích
          </h3>
          <textarea
            name="title"
        ...
        {/* Nút điều hướng - Ẩn khi đang sửa đơn vì đã có nút Nộp lại ở trang Revision */}
        {!isRevision && (
  ...
  // ===========================================================================
  // LOGIC RENDER LINH HOẠT
  // ===========================================================================
  
  // 1. Nếu là chế độ Sửa đơn (Revision): Chỉ trả về nội dung Form, không kèm Header/Sidebar
  if (isRevision) {
    return <div className="w-full animate-in fade-in duration-500">{FormContent}</div>;
  }

  // 2. Nếu là chế độ Nộp đơn mới: Trả về đầy đủ giao diện trang (Giao diện cũ của bạn)
  return (
          ...
          <div className="space-y-6">
            {steps.map((step) => (
              <div key={step.id} className="flex items-center gap-4 relative">
                ...
                {step.id !== 6 && <div className="absolute left-4 top-8 w-0.5 h-6 bg-gray-100 -z-0"></div>}
              </div>
            ))}
          </div>
        </aside>
