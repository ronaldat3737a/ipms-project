...
const DesignReviewList = () => {
  ...
  const filteredData = Array.isArray(designs) ? designs.filter(item => {
    if (item.appType !== 'KIEU_DANG_CN') return false;
  ...
  const handleRowClick = (item) => {
    // For now, all clicks go to the same review page for designs
    navigate(`/examiner/review/KIEU_DANG_CN/${item.id}`);
  };
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden font-sans text-slate-900">
      <aside className="w-72 bg-slate-900 text-white flex flex-col shadow-2xl shrink-0">
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-2">
          ...
          <SidebarItem icon={<FileText size={18} />} label="TĐ Sáng chế" onClick={() => navigate("/examiner/applications/sang-che")} />
          <SidebarItem icon={<Lightbulb size={18} />} label="TĐ Giải pháp hữu ích" onClick={() => navigate("/examiner/applications/giai-phap-huu-ich")} />
          <SidebarItem icon={<Package size={18} />} label="TĐ Kiểu dáng công nghiệp" active={true} onClick={() => navigate("/examiner/applications/KIEU_DANG_CN")} />
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="h-20 bg-white border-b flex items-center justify-between px-10 shadow-sm z-10 shrink-0">
          <h1 className="text-xl font-bold text-blue-800 uppercase tracking-tighter">Đơn đăng kí {pageTitle}</h1>
        ...
        <section className="flex-1 overflow-y-auto p-8 space-y-6">
          <div className="space-y-2">
            <h2 className="text-2xl font-bold text-slate-800">Danh sách đơn đăng ký {pageTitle}</h2>
            <p className="text-sm text-slate-500 font-medium">Quản lý các đơn đăng ký {pageTitle} từ PostgreSQL.</p>
          </div>
...
export default DesignReviewList;